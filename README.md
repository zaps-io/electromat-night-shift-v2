...
Cinematic WebGL lot: ACES Filmic (exposure ~0.78) + one warm dusk `DirectionalLight`, golden-hour `scene.environment` via `PMREMGenerator`, `postprocessing` bloom / vignette / SMAA. Twin cream canopies with a taller cream fascia, thicker red lip, column bolt plates, and official red Zaps wordmarks, 24 Slim Zeus (brushed metal that reads at dusk, charcoal recess, cyan base ring, amber PLUG IN, stall-number discs, plug buttons, tight CCS hang from holster to port), rubber parking stops on the six playable stalls (Zeus–bumper gap, not walk colliders), concrete pad joints on the charger islands, enterable glass store/lounge (OPEN south door + dusk storefront, warm interior spill, retail gondolas / coolers / robot cafe, L-sofa lounge, status board, wood/crete floor — no extra Reflectors), grit asphalt + two-tone curb islands, dusk neighborhood backdrop (brick / ribbon / glass tower / shop awnings / street trees). Guest cars are the Sketchfab **2018 Tesla Model 3** (Ameer Studio, CC BY 4.0) with paint forced opaque (`MeshPhysicalMaterial` clearcoat, `transmission: 0`) and dark opaque greenhouse glass. Stall X is `Zeus + half hull + 0.28m pedestal + 0.9m bumper gap` so cars do not interpenetrate Slim Zeus. Walker stays inside lot rails (no void) and can walk through the lounge door; furniture AABBs keep the pavilion from being a ghost box. No lofted placeholders, no Taycan helper cages, no see-through paint.
