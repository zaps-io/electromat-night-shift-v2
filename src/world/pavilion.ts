...
function addInterior(g: THREE.Group, W: number, D: number, H: number): THREE.Box3[] {
  ...
  const floor = new THREE.Mesh(
    ...
    mat(0xefe6d6, { roughness: 0.62, metalness: 0.04, envMapIntensity: 0.18 }),
  );
  ...
  const counter = new THREE.Mesh(new RoundedBoxGeometry(2.55, 0.96, 0.72, 2, 0.05), cream);
  counter.position.set(-2.55, 0.58, -3.15);
  const top = new THREE.Mesh(new THREE.BoxGeometry(2.62, 0.05, 0.78), charcoal);
  top.position.set(-2.55, 1.08, -3.15);
  const pos = box(0.28, 0.32, 0.08, charcoal, -1.7, 1.32, -3.4);
  const posGlow = new THREE.Mesh(new THREE.PlaneGeometry(0.22, 0.24), amber);
  posGlow.position.set(-1.7, 1.32, -3.45);
  g.add(counter, top, pos, posGlow);
  ...
  g.add(caseA, merchFace, snackCase, snackFace);

  for (const [x, y, z, c] of [
    [-3.35, 1.22, -3.05, C.red],
    [-3.05, 1.22, -3.05, C.cyan],
    [-2.75, 1.22, -3.05, C.amber],
  ...
  for (const [x, z] of [
    ...
    const disc = new THREE.Mesh(new THREE.CircleGeometry(0.22, 16), lamp);
    disc.rotation.x = Math.PI / 2;
    disc.position.set(x, H - 0.28, z);
    g.add(disc);
  }
  const strip = new THREE.Mesh(new THREE.BoxGeometry(W - 1.6, 0.04, 0.12), amber);
  strip.position.set(0, H - 0.22, -2.4);
  const strip2 = new THREE.Mesh(new THREE.BoxGeometry(W - 1.6, 0.04, 0.12), amber);
